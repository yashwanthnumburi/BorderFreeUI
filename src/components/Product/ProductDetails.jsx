...
export default class ProductDetails extends React.Component{
    ...
    getExpectedDeliveryMessage(){
        const date=new Date();
        date.setDate(date.getDate() + this.props.itemDetails.Details.ExpectedDelivery);
            ...
            <div className='row'>
                <div className='col-sm-12'>
                    ...
                    <div className='btnSizeGroup'>
                        <span><button className='btnSize' onClick={this.handleSizeClick()}>S<span className='itemLeft'>1 Left</span></button></span>
                        &nbsp;&nbsp;
                        <span><button className='btnSize' onClick={this.handleSizeClick()}>M<span className='itemLeft'>1 Left</span></button></span>
                        &nbsp;&nbsp;
                        <span ><button className='btnSize' onClick={this.handleSizeClick()}>L<span className='itemLeft'>1 Left</span></button></span>
                        &nbsp;&nbsp;
                        <span><button className='btnSize' onClick={this.handleSizeClick()}>XL<span className='itemLeft'>1 Left</span></button></span>
                    </div> */}
                    ...
                    <hr></hr> */}
                    <h6 style={{'marginBottom':'20px'}}>PRODUCT DETAILS</h6>
                    {this.props.itemDetails.Details.ProductDetails.split('-').map(spec=><p className='productSpecifications'>{spec}</p>)}
                    <h6 style={{'marginBottom':'20px'}}>SIZE & FIT</h6>
                    {this.props.itemDetails.Details['Size&Fit'].split('-').map(spec=><p className='productSpecifications'>{spec}</p>)}
                    <h6 style={{'marginBottom':'20px'}}>MATERIAL & CARE</h6>
                    {this.props.itemDetails.Details['Material&Care'].split('-').map(spec=><p className='productSpecifications'>{spec}</p>)}
                </div>
            </div>:''}
        </>
    }
